import { VisualElement, SHAPES, Panel, Button, BUT_GLOB, Slider, RadioGroup, Checkbox, ButInputs, MOUSE, ValFun, ClickFun } from "./button.js";
import { drawShape, drawRoundRect, drawTextBox } from "./tech.js";
import { arrFindMin, makeCopy, mmax, mmin, arrFill, arrFind, mabs } from "./math.js";



export function shapeOnButton(el:VisualElement, shape:number[][], sx:number, sy:number, sc:number,rot:number){
...
export class ScrollPanel<NAMES=any> extends Panel<NAMES>{
    innerw:number;
    innerh:number;
    wslider:Slider;
    hslider:Slider;
    ...
    wshift:number=0;
    sliderSz:number=10;
    innerCanv:HTMLCanvasElement=document.createElement('canvas');
    innerCtx:CanvasRenderingContext2D=this.innerCanv.getContext('2d');
    constructor(inp: ButInputs&{ sliderSz?:number}){
        super(inp);
        if(inp.sliderSz!=undefined)this.sliderSz=inp.sliderSz;
        this.innerw=this.w;
        this.innerh=this.h;
    ...
    getButtonIn(x:number,y:number):Button{  //override
        return super.getButtonIn(x+this.wshift,y+this.hshift);
    }
    getPanelIn(x:number,y:number):Panel{  //override
	...
	childMoved(ch:VisualElement){ if(this.children.length>2)this.checkBoundaries(); }
    checkBoundaries(){
		let maxx=-arrFindMin(this.children,(c)=>{return -(c.x+c.w)}).d;
		let maxy=-arrFindMin(this.children,(c)=>{return -(c.y+c.h)}).d;
		this.innerw=mmax(maxx,this.w);this.wslider.active=maxx>this.w;
		this.innerh=mmax(maxy,this.h);this.hslider.active=maxy>this.h;
    }
}

export class TabPanel<T=any> extends Panel<T>{
	panels:VisualElement[]=[];
	tabpan:Panel;
	tabbuts:Checkbox[]=[];
	tabRadGr:RadioGroup=new RadioGroup();
	panSz:number=15;
	labels:string[]=[];
	constructor(inp: ButInputs&{ labels?:string[], panSz?:number}){
			super(inp);
	this.tabpan=new Panel({par:this,x:0,y:0,w:this.w,h:this.panSz,name:'tabpan'});
			if(inp.labels)this.labels=inp.labels;
			if(inp.panSz)this.panSz=inp.panSz;
	}
	setXYW(inp: { x?: number; y?: number; w?: number; h?: number; z?: number;} = {}){
		super.setXYW(inp);
	...
	setTabs(pans:Panel[],labels:string[]=null){
		let n=pans.length;
		if(!labels)labels=arrFill('',n);
		for(let i=0;i<pans.length;i++)this.addTab(pans[i],labels[i]);
		this.choosePanel(0);
		this.tabbuts[0].domsclick(0,0,false);
	}
	addTab(pan:VisualElement,label:string){
    ...
    choosePanel(ind:number){
        for(let pan of this.panels)pan.active=false;
...
export class ObjectSlider<O=any> extends Slider{
	offX:number=0;
	// targetOffX:number=0;
	objSz:number;
	dragObj:boolean=false;
	insertpos:number=0;
	rearrange:boolean=true;
	msdownpos:number=null;
	set val(v){
		if(v>1)return;
	...
	objects:O[];
	drawObj:(ctx,x,y,w,h,obj,sel:boolean)=>void;
	get maxNum(){return this.objects.length;}
	set maxNum(v){}//console.log('empty set of maxNum '+v)}
	rearrangeFun:(oldi:number,newi:number,obj:O)=>void;
	constructor(inp:{ par: Panel; x?: number; y?: number; w?: number; h?: number; z?: number; msclick?: ClickFun; props?:any;canvN?:number; name?:string;// 
			val?: number; valFun?: ValFun; maxNum?: number; objects?:O[]; drawObj?:(ctx,x,y,w,h,obj:O,sel:boolean)=>void, horizontal?:boolean}&{objSz?:number, rearrange?:boolean,
			rearrangeFun?:(oldi:number,newi:number,obj:O)=>void}){
		super(inp);
		let {objects=[],drawObj= null, horizontal=null, rearrange=null,objSz=30}=inp;
		this.objSz=objSz;
		if(horizontal!=null)this.horizontal=horizontal;
		if(rearrange!=null)this.rearrange=rearrange;
		if(inp.rearrangeFun)this.rearrangeFun=inp.rearrangeFun;
		...
		// this.val=0;
			this.drawMe=function(ctx){
				if(this.dragObj){
					...
					else ctx.fillRect(0,this.insertpos*this.objSz-this.offX,this.w,2);
				}
			}
		...
		this._msmove=function(x,y,rmb){
			...
			if(!this.dragObj){
				if((yx>hw||yx<0) && this.rearrange){
					...
					if(xy<this.objSz)this.offX=mmax(0,mmin(this.objects.length*this.objSz-wh,this.offX-this.objSz*.3));;
					if(xy>wh-this.objSz)this.offX=mmax(0,mmin(this.objects.length*this.objSz-wh,this.offX+this.objSz*.3));;
		...
		this._msup = (mx,my,rmb)=>{
			...
			if(this.dragObj){
				this.dragObj=false;
				if(this.insertpos>this.pos)this.insertpos--;
				let el=this.objects.splice(this.pos,1);
				this.objects.splice(this.insertpos,0,el[0]);
				let oldpos=this.pos;
				this.pos=this.insertpos;
				if(this.rearrangeFun)this.rearrangeFun(oldpos,this.insertpos,this.selObj);
				this.drawCanv();
			}
	...
	drawCanv(){
		this.drawAllCanv((self,ctx,ind)=>{
			...
			if(!this.horizontal){xx=0;yy=this.objSz;offx=0;offy=this.offX;ww=self.w;hh=this.objSz;}
			for(let i=0;i<this.objects.length;i++){
				if(this.pos!=i || !this.dragObj)this.drawObj(ctx,(i)*xx-offx,i*yy-offy,ww,hh,this.objects[i],this.pos==i);
			}
		},false);
	}
	addObject(obj:O,ind:number=this.objects.length){
		this.objects.splice(ind,0,obj);
		// if(this.objects.length>1)this.pos=this.objects.indexOf(this.selObj);//this doesn't work
		this.drawCanv();
	}
	delObject({obj=null, ind=null}:{obj?:O,ind?:number}){
		if(obj)ind=this.objects.indexOf(obj);
		if(ind==null || ind<0)return;
		this.objects.splice(ind,1);
		if(this.selObj)this.pos=this.objects.indexOf(this.selObj);
		this.drawCanv();
	}
	clearObjects(){
		this.objects=[];
	...
	clickPos(pos:number,rmb:boolean=false){//override
	...
	selectObject(obj:O){
		...
		if(ind>=0)this.clickPos(ind,false);
	}
}
...
export class StringEditor extends Button{
	chars:string[]=[];
	savedChars:string[]=[];
	get line():string{return this.chars.join('')}
	letW:number=7;
	letH:number=13;
	lpos:number=~~(this.chars.length/2);
	lpos2:number;
	offsetX:number=0;
	selected:boolean=false;
	enterFun:(str:string)=>void=null;
	get doenterFun(){return this._enterFun};
	protected _enterFun:(s:string)=>void = (s)=>{if(this.enterFun)this.enterFun(s);}
	escFun:()=>void=null;
	get doescFun(){return this._escFun};
	protected _escFun:()=>void = ()=>{if(this.escFun)this.escFun();}
	constructor(inp:ButInputs & {enterFun?:(str:string)=>void, initStr?:string}){
		super(inp);
		if(inp.enterFun)this.enterFun=inp.enterFun;
		this.canvN=1;
		this.crtAllCanv();
		...
		this.msmove=(mx,my,rmb)=>{
			...
			if(this.offsetX+this.lpos2*this.letW<0)this.offsetX+=this.letW;
			this.fillCanvases();
		...
		this.drawMe=(ctx)=>{
			if(BUT_GLOB.keybFocusBut==this){
				if(~~(BUT_GLOB.stepN/50)%2==0)ctx.fillRect(this.offsetX+this.lpos*this.letW-2,0,2,this.letH+3);
			}
		}
		this.kbdown=(e)=>{
			let key=e.key;
			...
			if(key.length==1 && (key>='a' && key<='z' ||key>='A' && key<='Z' ||key>='0' && key<='9' || ['(',')','[',']','+','*','-','/','.',' ','~','<','>','=','_'].indexOf(key)>=0)){
				...
				if(key=='Backspace' && this.lpos>0 || key=='Delete' && this.lpos<this.chars.length){
					...
					switch(key){
						case 'ArrowRight':this.lpos2=this.lpos=mmin(this.chars.length,mmax(this.lpos,this.lpos2)+(this.lpos==this.lpos2?1:0));
							break;
						case 'ArrowLeft':this.lpos2=this.lpos=mmax(0,mmin(this.lpos,this.lpos2)-(this.lpos==this.lpos2?1:0));
							break;
						case 'Escape':
		...
		if(inp.initStr)this.setLine(inp.initStr);
	}
	loseKbFocus(){
		console.log(this.selected);
		
		if(this.selected){this.selected = false; this.doenterFun(this.line);}
	}
	setLine(s:string){
		this.savedChars=makeCopy(this.chars);
		this.chars=s.split('');
	...
	fillCanvases(){
		...
		if(this.w>this.chars.length*this.letW)this.offsetX=(this.w-this.chars.length*this.letW)/2;
		else
		 if(this.lpos2*this.letW+this.offsetX>this.w-border)this.offsetX-=this.letW;//this.w-(this.lpos*this.letW+this.offsetX)-border;
			else if(this.lpos2*this.letW+this.offsetX<border || this.chars.length*this.letW+this.offsetX<this.w-border-this.letW)this.offsetX+=this.letW;//(this.lpos*this.letW+this.offsetX)-border;
		this.drawAllCanv((self,ctx,ind)=>{
...
export class Namer extends StringEditor{
	constructor(){
		...
		this.active = false;
		this._enterFun= (s)=>{if(this.enterFun)this.enterFun(s);this.active = false;}
		this.setXYW({z:-100});
	...
	static callNamer(scrX:number,scrY:number,w:number,h:number,enterFun:(str:string)=>void,strtLine:string=''){
...
export function panelSqueezeLayout(pan:Panel, lout:any[],styles:LayoutEl[]=[{el:null}]){
    lout = setStyles(lout,styles);
    layoutSqueeze(lout, pan.w,'x',0);
    if(!Array.isArray(lout))layoutSqueeze(lout, pan.h,'y',0);
    else lout.forEach((el)=>layoutSqueeze(el,pan.h,'y',0));
    function setStyles(lout,styles){
        if(Array.isArray(lout))for(let i in lout)lout[i]=setStyles(lout[i],styles);
        else if(lout.style!=undefined){let ll=makeCopy(styles[lout.style]); ll.el = lout.el; lout=ll;}
        else if(lout instanceof VisualElement){let ll=makeCopy(styles[0]); ll.el = lout; lout=ll;}
        return lout;
    }
}
let xytowh={x:'w', y:'h'}
function layoutSqueeze(lout:LayoutRect,sz:number,dim:string='x', coord:number=0){
    setFix(lout);
    ...
    function setFix(lout){            
        if(Array.isArray(lout))lout.forEach((el)=>setFix(el));    
        else if(lout['fix'+dim]!=undefined){let inp={};inp[xytowh[dim]]=lout['fix'+dim];lout.el.setXYW(inp);}
    }
}
function getSz(lout:LayoutRect, dim:string):number{
    let sum =0, maxo;
    if(!Array.isArray(lout))return (lout.el[xytowh[dim]]);
    else
        for(let el1 of lout)
            if(!Array.isArray(el1)){sum+=(el1.el[xytowh[dim]]);}//if(el1['fix'+dim]==undefined)
            else {
                maxo=arrFindMin(el1,(e)=>(-getSz(e,dim)));
                el1.forEach((el2,i)=>{
                    if(i==maxo.i)return;
                    if(!Array.isArray(el2)){stretch(el2,dim,-maxo.d/el2.el[xytowh[dim]],el2.el[dim],);}
                    else layoutSqueeze(el2,-maxo.d,dim);
                });
                sum+=-maxo.d;
            }
    return sum;
}
function stretch(lel:LayoutEl, dim:string, perc:number,coord:number){
    let inp={};
    if(lel['stretch'+dim])inp[xytowh[dim]]=lel.el[xytowh[dim]]*perc;
    inp[dim]=coord;
    lel.el.setXYW(inp);
}
export type LayoutRect = ( LayoutEl | LayoutRect[]);
type LayoutEl = {el:VisualElement, stretchx?:boolean, stretchy?:boolean, fixx?:number, fixy?:number, style?:number};