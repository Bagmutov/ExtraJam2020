...
import { FieldObject, Room, Player, Natto, SmartFace } from "./objects.js";
import { Cosmetics, backCanv, backctx } from "./cosmetics.js";
import { dist, mabs } from "./math.js";
export let scrw = 640, scrh = 480;
export let mainCanv = document.getElementById('can');
...
export let roomh = scrh, roomw = scrw, allRooms = [[]]; //(can.width,can.height,0,0)]];//
export let allObjects = [], toDestroy = [];
let msdCrd = null;
export let bigText = { str: null, t: null };
...
//   }
export let actRoom = { x: null, y: null }, camx = 0, camy = 0, camvx = 0, camvy = 0;
export let colliderMain = new Collider(~~(roomw * 3 / 200) + 1, ~~(roomh * 3 / 200) + 1, 200, [Collider.commonPutters.putRectMov, Collider.commonPutters.putLine], [[Collider.commonCheckers.circToCirc, Collider.commonCheckers.circToLine2], [Collider.commonCheckers.circToLine2, Collider.commonCheckers.falser]]);
initLevel();
export let player = new Player(1, 1, 0, 0, 30, 100);
crtRoom(0, 0);
...
function worldStep() {
    ...
    while (i < lst.length) {
        ...
        if (colTupl.objs.length > 0) {
            ...
            if (colTupl.objs.length > 0) {
                let obj1 = lst[i].obj, obj2 = colTupl.objs[0].obj;
                ...
                if (bounce) {
                    ...
                    if (imp == null)
                        colliderMain.unintersect(colTupl.objs[0], lst[i], colTupl.cols[0]);
                }
                colliderMain.putOnGrid(lst[i]);
...
function createNatto(x, y, vx, vy) {
...
function keyUp(e) {
    switch (e.key) {
        case ' ':
            player.doeating = false;
            player.mouth.tp = 0;
            player.eyes.forEach(e => e.tp = 0);
            player.doeating = false;
            break;
        case 'w':
            player.control.w = 0;
...
export function crtRoom(crdx, crdy) {
    let ro;
    if (!allRooms[crdy])
        allRooms[crdy] = [];
...
function addWall(rx, ry, x1, y1, x2, y2, clr = undefined) {
    let w = new Line(x1, y1, x2, y2); //{x1:x1,y1:y1,x2:x2,y2:y2};
    level[rx][ry].walls.push(w);
    if (clr)
        w.color = clr;
    lastWall = { w, rx, ry };
}
function connectWall(x, y) {
    if (!lastWall)
