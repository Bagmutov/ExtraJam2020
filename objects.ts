import { drawCircle, drawTextBox, drawTriangle } from "./tech.js";
import { allRooms, crtRoom, allObjects, colliderMain, actRoom, roomw, roomh, mainCanv, camx, setCam, camvx, camy, Point, toDestroy, scrw, scrh, bigText } from "./main.js";
import { Cosmetics, backctx, backCanv } from "./cosmetics.js";
import { sign, dist2Pt, mmin, mmax, arrFindMin, dist2, arrSortOne } from "./math.js";
import { Line } from "./collider.js";

export class FieldObject{
  get c_x(){return this.x+this.vx}
  get c_y(){return this.y+this.vy}
  room:Room;
  fx:number=0;fy:number=0;
  color:string='#444444';
  edible:boolean=false;
  constructor( public x:number,  public y:number,  public vx:number, public vy:number,public rad:number, public mass:number){
  ...
  deactivate(){
    ...
    if(ind>=0)allObjects.splice(ind,1);
    else console.log('error del obj from all');
  }
  setRoom(ro:Room){
    this.room=ro;
  }
  jumpCoord(dx:number,dy:number){
    this.x+=dx;
    this.y+=dy;
  }
  collide(obj:FieldObject):boolean{
    return true;
  }
  destroy(){
    if(this.room) this.room.delObj(this);
  }
}
...
export class Natto extends FieldObject{
  ...
  maxD2:number=40000;
  minD2:number=8000;
  constructor( x:number,  y:number,  vx:number, vy:number,rad:number, mass:number){
  ...
  step(){
    super.step();
    let dx:number,dy:number, cn:FieldObject, i=0,dd:number;
    while(i<this.conns.length){
      cn=this.conns[i];
      dd=dist2Pt(cn,this);
      if(dd>this.maxD2)this.conns.splice(i,1);
      else {
  ...
  collide(obj:FieldObject):boolean{
    if(this.conns.indexOf(obj)>=0)return;
    // if(this.conns.length==this.maxCon)//
    if(!(obj instanceof Natto))this.conns.pop();
    if(this.conns.length<this.maxCon)
      this.connectTo(obj);
    return true;
  }
  connectTo(obj:FieldObject){
    if(this.conns.length>=this.maxCon)this.conns.pop();
    this.conns.push(obj);
  }
}
...
export class Face extends FieldObject{
  eyes:Eye[]=[];
  mouth:Mouth=new Mouth(-20,0);
  randtimer:number=10;
  lookhim:Point=null;
  border:Follower[]=[];
  emotimer:number=-1;
  doeating:boolean=false;
  monolog:string[]=['Hello, nice to meet you!','now go'];

  constructor( x:number,  y:number,  vx:number, vy:number,rad:number, mass:number, eyeN:number=~~(Math.random()*4)+1){
    super(x,y,vx,vy,rad,mass);
  ...
  step(){
    ...
    if(this.lookhim)this.lookAt(this.lookhim);
    if(--this.emotimer==0){this.defaultEmo()}
    // console.log(this.food);
    
  }
  defaultEmo(){
    this.mouth.tp=0;this.eyes.forEach(e=>e.tp=0);
  }
  collide(obj:FieldObject):boolean{
  ...
  eat(obj:FieldObject){
    if(obj instanceof Natto){obj.conns.forEach(cn=>{if(cn instanceof Natto)cn.connectTo(this)})}
    if(obj.rad--<=5)toDestroy.push(obj);
  }
  lookAt(obj:Point){
    for(let ey of this.eyes)
      ey.lookAt(obj.x-this.x,obj.y-this.y);
  }
  spit(sin:number,cos:number,pow:number,objs:FieldObject[]){
    for(let o of objs){
...
class Eye extends Follower{
  public irdx:number=20;
  public irdy:number=20;
  constructor(public x:number,public y:number,public tp:number){
    super(x,y);
  }
  drawMe(ctx:CanvasRenderingContext2D){
...
function getRndClr():string{
  return toClr(getRndClrN())
  function getRndClrN():number{	//rand Number
    var n=~~(Math.random()*0xffffff);
    return n;
  }
  function toClr(n:number):string{
...
export class Player extends Face{
  control:{w:number,a:number,s:number,d:number}={w:0,a:0,s:0,d:0};
  constructor( x:number,  y:number,  vx:number, vy:number,rad:number, mass:number){
    super(x,y,vx,vy,rad,mass);
    this.color='#aa6666';
  }
...
type Task={nm:string,step:(p)=>void,cond:(p)=>number,prop,init:Function};
export class SmartFace extends Face{
  tasks:{ts:Task,p:any,time:number}[]=[];
  plLove:number;
  excitement:number=0;

  constructor({ x=0, y=0, vx=0, vy=0, rad, mass=rad*rad*.01, eyeN=1,color=getRndClr()}: { x?: number; y?: number; vx?: number; vy?: number; rad: number; mass?: number; eyeN?: number; color?:string}){
  ...
  step(){
    ...
    }else this.reactTo('idle',null);
  }
  addTask(ts:Task,args:any[]=[]){
    this.tasks.push({ts:ts,p:ts.init.apply(this,args),time:1});
  }
  reactTo(tp:'idle'|'plcoll'|'facecoll'|'seepl'|'seeobj',oth:FieldObject){
    switch(tp){
      case 'idle': this.addTask(this.commonTasks.idle); break;
      // case '': break;
    }
  }
  commonTasks={
    goto:{nm:'goto',prop:{tx:0,ty:0,tm:0},cond:(p)=>{if(p.tm++>=10)return 0;if(dist2(this.x-p.tx,this.y-p.ty)<100)return 0;else return 20;},
          step:(p)=>{
...
export class Room{
  ...
  constructor(public w:number, public h:number, public crdX:number,public crdY:number){
  }
  step(){
    // for(let o of this.objects)o.step();
    let i=0,o:FieldObject;
    while(i<this.objects.length){
      o=this.objects[i];
      if(o.x>this.w+this.position.x)this.moveObjTo(o,1);
      else if(o.x<this.position.x)this.moveObjTo(o,3);
      else if(o.y>this.h+this.position.y)this.moveObjTo(o,2);
      else if(o.y<this.position.y)this.moveObjTo(o,0);
      else i++;
    }
  }
  ...
  draw(ctx:CanvasRenderingContext2D){
    ...
    for(let w of this.walls){
        if(w.color)ctx.strokeStyle=w.color;
  ...
  activate(posx:number,posy:number){
    this.active=true;
    for(let o of this.objects){
      o.x+=posx;
      o.y+=posy;
    ...
    this.walls.forEach(w=>{      
      w.x1+=posx;w.x2+=posx;      w.y1+=posy;w.y2+=posy;
  ...
  // }
  deactivate(){//posx:number,posy:number
    this.active=false;
    for(let o of this.objects){
      o.x-=this.position.x;
      o.y-=this.position.y;
    ...
    this.walls.forEach(w=>{      
      w.x1-=this.position.x;w.x2-=this.position.x;      w.y1-=this.position.y;w.y2-=this.position.y;
      colliderMain.delObj({obj:w});
    
    })
    this.position=null;
  }
  addObj(obj:FieldObject){
    ...
    if(obj.room)console.log('Error room of obj');
    if(this.active)obj.activate();
    obj.setRoom(this);
  }
  delObj(obj:FieldObject){
    let ind=this.objects.indexOf(obj);
    if(ind>=0)this.objects.splice(ind,1);
    else console.log('Error obj room');
    if(this.position){
      obj.x-=this.position.x;
      obj.y-=this.position.y;
    }
    obj.room=null;
    if(this.active)obj.deactivate();
  }
  addWall(wa:Line){
  ...
  moveObjTo(obj:FieldObject,dir:0|1|2|3){
    this.delObj(obj);
    switch(dir){
      case 2:obj.jumpCoord(0,-this.h);  break;
      case 3:obj.jumpCoord(+this.w,0);  break;
      case 1:obj.jumpCoord(-this.w,0);  break;
      case 0:obj.jumpCoord(0,+this.h);  break;
    }
    let dj=(2-dir)%2,di=(dir-1)%2;
    this.checkNeib(this.crdY+di,this.crdX+dj);
    allRooms[this.crdY+di][this.crdX+dj].addObj(obj);
  }
  checkNeib(i:number,j:number){
    if(!allRooms[i]||!allRooms[i][j])crtRoom(j,i);
  }
  // connectTo(dir:Dir){
  //   let di=(dir-1)%2+1,dj=(dir-2)%2+1;
  //   allRooms[this.crdY+di][this.crdX+dj]=;
  //   ro.neib[-di][-dj]=this;
  // }
}

export function setCentralRoom(rx:number,ry:number){
  if(actRoom.x!=null)
    for(let i=-1;i<2;i++)
      for(let j=-1;j<2;j++)
  ...
  for(let i=-1;i<2;i++)
    for(let j=-1;j<2;j++){
      if(!allRooms[ry+i]||!allRooms[ry+i][rx+j])crtRoom(rx+j,ry+i);
